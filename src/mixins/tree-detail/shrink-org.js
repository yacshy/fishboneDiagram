import SpreadOrg from './spread-org'
...
export default class ShrinkOrg extends SpreadOrg {
    ...
    #shrink_branch_org(b2Start, OLD_INFO, index) {
        ...
        /**org  列表 */
        const OLD_ORG = OLD_INFO.data[index].org
        /**org trunk  高度 */
        const OLD_TRUNK_HEIGHT = OLD_ORG.length * 32 /**每个组织和人员占位 */ + 12/** 超出 */
        const NEW_TRUNK_HEIGHT = 12/** 超出 */
        /**org trunk 起点 */
        const trunkStart = {
            y: orgStart.y + 16,
            x: orgStart.x + (isLeft ? -36 : 36)
        ...
        Raf(OLD_TRUNK_HEIGHT, NEW_TRUNK_HEIGHT, (offset) => {
            trunk.attr('points', BranchOrgTrunkPoints(trunkStart, offset, true))
        })
        // 收起附属枝丫
        OLD_ORG?.forEach((_, orgIndex) => {
    ...
    shrinkOrg(index) {
        ...
        this.templateShadw.data[index].org = []
        this.updateCurrentData()
        $(`[mark="org_ball_${index}"]`).off().on('click', () => {
            this.spreadOrg(index)
        })
        ...
        /**branch org的起点 */
        this.#shrink_branch_org(b2Start, OLD_INFO, index)
    }
}