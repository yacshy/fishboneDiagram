import ShrinkBranch from './shrink-branch'
...
export default class SpreadOrg extends ShrinkBranch {
    ...
    #translation(current) {
        ...
        this.currentData.data.forEach((info, index) => {
            ...
            /**新的起点 branch 1 的起点 */
            const start = { x: this.root.x + info.addup / Tan(75), y: this.root.y - info.addup }
            const ballPosi = {
    ...
    #spread_org(start, index) {
        ...
        // 修改ball里的内容
        $(`[mark="org_ball_${index}"]`).css(ballPosi)
        $(`[mark="org_ball_${index}_text"]`).attr({
            x: ballPosi.x + 3.5,
            y: ballPosi.y + 9.5
        })
        ...
        /**branch-org-trunk高度 */
        const TrunkHeight = myOrg.length * 32 /**每个组织和人员占位 */ + 12/** 超出 */
        /**生长 branch_org 的trunk，然后出现 name (即：“组织”) */
        const trunk = $(`[mark="org_${index}_trunk"]`)
        Raf(0, TrunkHeight, (offset) => {
            trunk.attr('points', BranchOrgTrunkPoints(orgTrunkStart, offset, true))
        })
        $(`[mark="org_${index}"]`).attr({ 
            x: orgStart.x + (isLeft ? -75 : 75), 
            y: orgStart.y + 20
         })
        // 生长 branch_org 下的附枝
    ...
    spreadOrg(index) {
        this.templateShadw.data[index].org = simpleDeepClone(this.template.data[index].org)
        const OLD_DATA = simpleDeepClone(this.currentData)
        const data = this.updateCurrentData()
        $(`[mark="org_ball_${index}"]`).off().on('click', () => {
            this.shrinkOrg(index)
        })
        $(`[mark="org_ball_${index}_text"]`).off().on('click', () => {
            this.shrinkOrg(index)
        })
        const { x, y } = this.root
        ...
        const start = { x: this.root.x + data.data[index].addup / Tan(75), y: this.root.y - data.data[index].addup }
